function gaToolExample()
%% 采用遗传算法工具箱对peaks函数进行寻优计算
    clc, clear, close all;
    warning off;
    
    % Nonlinear Constraint Function
    % Subject to a nonlinear constraint defined by a circular region of
    % radius three around the origin type circularConstraint
...
%                                                     @peaksPlotIterates));
    ...
    problem.lb = [-3 -3];
    problem.ub = [ 3  3];
    problem.options = gaoptimset('PopInitRange', [-3 -3;3 3],...
...
%% Nonlinear constraint definition
    ...
    % circular region with radius 3: x1^2+x^2-9<=0
    c = x(:,1).^2 + x(:,2).^2 - 9;
end
