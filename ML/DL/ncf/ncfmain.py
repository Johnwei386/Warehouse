...
#from ncf import NCF, metrics, NCF_input
import NCF_input
import metrics
import NCF

# FLAGS用来传递参数:('变量名',默认值,'描述')
...
def train(train_data,test_data,user_size,item_size):
    with tf.Session() as sess:
        ...
        model = NCF.NCF(FLAGS.embedding_size, # 16
                        user_size, # 1508
                        item_size, # 2071
                        FLAGS.lr,  # learning
                        FLAGS.optim, # Adam
                        FLAGS.initializer, # Xavier
                        FLAGS.loss_func, # cross_entropy
                        FLAGS.activation,# Relu
                        FLAGS.regularizer, # the regularizer rate
                        iterator, 
                        FLAGS.topK, # 10
                        FLAGS.dropout, # dropout rate
                        is_training=True)
        ...
        for epoch in range(FLAGS.epochs):
            ...
            try:
                ...
                print("Epoch %d training " % epoch + "Took: " + time.strftime("%H: %M: %S", time.gmtime(time.time() - start_time)))
            ...
            try:
                ...
                print("Epoch %d testing  " % epoch + "Took: " + time.strftime("%H: %M: %S", time.gmtime(time.time() - start_time)))
...
def main():
    ...
    train_data = NCF_input.train_input_fn(train_features,
                                          train_labels,
                                          FLAGS.batch_size,
                                          user_negative,
                                          FLAGS.negative_num)
    
    test_data = NCF_input.eval_input_fn(test_features, 
                                        test_labels,
                                        user_negative, 
                                        FLAGS.test_neg)
